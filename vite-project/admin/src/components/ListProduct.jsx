...
import {TbTrash} from "react-icons/tb"
 ...
 const ListProduct = () => {
  ...
  const fetchInfo = async () => {
    await fetch(`http://localhost:2000/allproducts`).then((res) => res.json()).then((data)=> {setAllproducts(data)});
  }
useEffect(()=> {
  fetchInfo();
}, [])
...
const remove_product = async (id)=> {
  ...
  return (
    <div className='p-2box-border bg-white mb-0 rounded-sm w-full mt-4 sm:p-4 sm:m-7'>
